...
//echo '<pre>'; print_r($arParams["CACHE_TYPE"]); echo '</pre>';
...
//	);
	...
	//echo '<pre>'; print_r($arResultModules); echo '</pre>'; die();
	...
	$arSelect = array(
		...
		"DETAIL_PICTURE",
		"PREVIEW_PICTURE",
		"PROPERTY_OSTATOK_U_POSTASHCHIKA",
        "PROPERTY_FREE_PREPARE",
        "PROPERTY_FREE_SHIP",
        "PROPERTY_".$arParams['ARTICUL_PROP'],
        "PROPERTY_OLD_PRICE"
	);
	//if ($bIBlockCatalog)
	...
	//EXECUTE
/*
 * получаем массив с доступными статусами
 * nfzakirov@1cbit.ru Наиль Закиров 13.04.2016
 */

	$BIT_VISIBLE_STATUS_IBLOCK_ID=13256;
	switch($arParams['IBLOCK_ID']){
		case 9:$BIT_VISIBLE_STATUS_IBLOCK_ID=13256;break;
		case 13:$BIT_VISIBLE_STATUS_IBLOCK_ID=13257;break;
		case 15:$BIT_VISIBLE_STATUS_IBLOCK_ID=13258;break;
		default:break;
	}
	$bit_Field_Status=array();
	$tmp_arSelect = Array("ID","PROPERTY_BIT_STATUS_VALUE");
	$tmp_arFilter = Array("IBLOCK_ID"=>21, "ACTIVE_DATE"=>"Y", "ACTIVE"=>"Y","=PROPERTY_BIT_INFO"=>$BIT_VISIBLE_STATUS_IBLOCK_ID);
	$res = CIBlockElement::GetList(Array(), $tmp_arFilter , false, false, $tmp_arSelect);
	while($arFields = $res->Fetch())
	{
		//$arFields = $ob->GetFields();
		$bit_Field_Status[]=$arFields;
	}

$del_count=0;
/*----Конец получения массива со статусами----*/

$bit_form_arrFilter=array();
foreach($bit_Field_Status as $bit_item){
	$bit_form_arrFilter[]=$bit_item['PROPERTY_BIT_STATUS_VALUE_VALUE'];
}

$bitAtrFilter=array("PROPERTY_OSTATOK_U_POSTASHCHIKA_VALUE"=>$bit_form_arrFilter);

	$rsElements = CIBlockElement::GetList($arSort, array_merge($arrFilter, $arFilter,$bitAtrFilter), false, $arNavParams, $arSelect);
...
/*
 * Определение по статусу доступности показа
 * nfzakirov@1cbit.ru Наиль Закиров 13.04.2016
 */
/*$bit_visivle=false;
$del_count++;
...
foreach($bit_Field_Status as $bit_item){
	if($arItem['PROPERTY_OSTATOK_U_POSTASHCHIKA_VALUE'] == $bit_item['PROPERTY_BIT_STATUS_VALUE_VALUE']){
		$bit_visivle=true;
		$del_count--;
		break;
	}
...
//$arItem['VISIBLE']=$bit_visivle;
/*---Конец определения статса---*/
        ...
        foreach($arItem as $keyValue => $ItemValue)
        {
            if(strpos($keyValue, 'PROPERTY') !== false && $keyValue != 'IPROPERTY_VALUES')
            {
                $newphrase = str_replace(array('PROPERTY_', '_VALUE_ID', '_ENUM_ID', '_VALUE'), array('', '', '', ''), $keyValue);
                if(strpos($keyValue, '_VALUE_ID') !== false)
                {
                    $arItem['PROPERTIES'][$newphrase]['VALUE_ID'] =  $ItemValue;
                }
                elseif(strpos($keyValue, '_ENUM_ID') !== false)
                {
                    $arItem['PROPERTIES'][$newphrase]['ENUM_ID'] = $ItemValue;
                    $arItem['PROPERTIES'][$newphrase]['VALUE_ENUM_ID'] = $ItemValue;
                }
                elseif(strpos($keyValue, '_VALUE') !== false)
                {
                    $arItem['PROPERTIES'][$newphrase]['VALUE'] = $ItemValue;
                    $arItem['PROPERTIES'][$newphrase]['VALUE_ENUM'] = $ItemValue;
                }
                if($newphrase == $arParams['ARTICUL_PROP'])
                    $arItem['PROPERTIES'][$newphrase]['NAME'] = 'Артикул';
                //$arItem['PROPERTIES'][$newphrase] =
            }
        }
