				...
				if (empty($arResult["ERROR"]))
				{
					...
					foreach($arResult["PAY_SYSTEM"] as $arPaySystem)
					{
						if ($arPaySystem["CHECKED"]=="Y" && !($arParams["ONLY_FULL_PAY_FROM_ACCOUNT"] == "Y" && $arResult["USER_VALS"]["PAY_CURRENT_ACCOUNT"]=="Y")){ 
							$TYPE_PRICE=$arPaySystem["PSA_NAME"];
						}
					...
					$arFields = array(
						...
						"USER_NAME"=>$arUserResult["ORDER_PROP"][1],
						"USER_PHONE"=>$arUserResult["ORDER_PROP"][3],
						"USER_ADDRESS"=>$arUserResult["ORDER_PROP"][7],
