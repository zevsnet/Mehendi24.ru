...
foreach($arQuery as $key => $query)
{
    ...
    if($arLink['LINK'])
    {
        ...
        $arLink['DES'] = getArrDes($arLink['LINK'], $domane_Site);
        //if($arLink['IMAGES'])
        if($arLink['DES'])
        {
            SetUpdateProductDes($arLink);
 ...
 *Вытягиваем Описание
 */
function getArrDes($arLink, $domain)
{
    $offSite = file_get_contents($arLink);
    ...
    foreach(pq('.bottom_item') as $lightbox)
    {
        return pq($lightbox)->html();
    }
 ...
 *Запись Описания
 */
function SetUpdateProductDes($product)
{
