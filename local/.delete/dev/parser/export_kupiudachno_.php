...
//$url = "http://www.kupiudachno.ru/index.php/gel-lak-shellac-bluesky/shellac-bluesky-seriya-brazil/";
$url = "http://www.kupiudachno.ru/index.php/gel-lak-shellac-bluesky/shellac-bluesky-seriya-romantic.html?start=".$_REQUEST['start'];
$arResult = getName($url);
...
function getName($url)
{
    ...
    $exportItem = array();
    $arResult = array();
    foreach(pq('.mainblock') as $mainblock)
    {
        $res = array();
        $res['NAME'] = pq(pq($mainblock)->find('.name .product_title a'))->html();
        $res['DES'] = pq(pq($mainblock)->find('.description'))->html();
        $arResult[] = $res['NAME'] . $res['DES'];
    }
