...
class CModuleOptions
{
    ...
    public function CModuleOptions($module_id, $arTabs, $arGroups, $arOptions, $need_access_tab = false)
    {
        ...
        if($need_access_tab)
            $this->arTabs[] = array(
                'DIV' => 'edit_access_tab',
                'TAB' => GetMessage("blackbutterfly.seoredirektprod7_SETTINGS_ACESS"),
                'ICON' => '',
                'TITLE' => GetMessage("blackbutterfly.seoredirektprod7_SETTINGS_ACESS_DESC")
            );        
    ...
    public function ShowHTML()
    {
        ...
        if(is_array($this->arOptions))
        {
            ...
            $tabControl->Begin();
            echo '<form name="'.$this->module_id.'" method="POST" action="'.$APPLICATION->GetCurPage().'?mid='.$this->module_id.'&lang='.LANGUAGE_ID.'&mid_menu=1" enctype="multipart/form-data">'.bitrix_sessid_post();
            ...
            echo     '<input type="hidden" name="update" value="Y" />
                    <input type="submit" class="adm-btn-save" name="save" value="'.GetMessage("blackbutterfly.seoredirektprod7_SETTINGS_SAVE").'" />
                    <input type="reset" name="reset" value="'.GetMessage("blackbutterfly.seoredirektprod7_SETTINGS_CANSEL").'" />
                    </form>';
