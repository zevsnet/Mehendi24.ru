...
if($this->startResultCache(false, array($arrFilter, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $arNavigation, $pagerParameters)))
{
	...
	$arGroup = array('PROPERTY_' . $arParams['GROUP_BY']);

	//группируем в торговые предложения
	$rsGroup = CIBlockElement::GetList($arSort, array_merge($arrFilter, $arFilter), $arGroup, $arNavParams);
	while($arItem = $rsGroup->GetNext())
	{
		$arrFilter['PROPERTY_' . $arParams['GROUP_BY']][] = $arItem['PROPERTY_' . $arParams['GROUP_BY'] . '_VALUE' ];
	}
	...
	//EXECUTE
	$rsElements = CIBlockElement::GetList(array_merge(array('property_IS_PRODUCT' => 'desc'), $arSort), array_merge($arrFilter, $arFilter), false, false, $arSelect);
