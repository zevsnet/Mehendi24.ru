...
$arResult['CATALOG_QUANTITY'] = (0 < $arResult['CATALOG_QUANTITY'] && is_float($arResult['CATALOG_MEASURE_RATIO']) ? (float)$arResult['CATALOG_QUANTITY'] : (int)$arResult['CATALOG_QUANTITY']);
...
if($productSlider)
{
    ...
    {
        $productSlider[$i] = array_merge($arImage, array(
            "BIG"   => array('src' => CFile::GetPath($arImage["ID"])),
            "SMALL" => CFile::ResizeImageGet($arImage["ID"], array(
                "width"  => 340,
                "height" => 340
            ), BX_RESIZE_IMAGE_PROPORTIONAL, true, array()),
            "THUMB" => CFile::ResizeImageGet($arImage["ID"], array(
                "width"  => 56,
                "height" => 56
            ), BX_RESIZE_IMAGE_PROPORTIONAL, true, array()),
        ));
...
if($arResult['CATALOG'] && isset($arResult['OFFERS']) && !empty($arResult['OFFERS']))
{
        ...
        foreach($arResult['OFFERS'] as &$arOffer)
        {
            foreach($arSKUPropIDs as &$strOneCode)
            {
                if(isset($arOffer['DISPLAY_PROPERTIES'][$strOneCode]))
                {
                    ...
                    $valueId = ($arSKUPropList[$strOneCode]['PROPERTY_TYPE'] == Iblock\PropertyTable::TYPE_LIST ? $arOffer['DISPLAY_PROPERTIES'][$strOneCode]['VALUE_ENUM_ID'] : $arOffer['DISPLAY_PROPERTIES'][$strOneCode]['VALUE']);
        ...
        if($arOffer['MORE_PHOTO'])
        {
            foreach($arOffer['MORE_PHOTO'] as $i => $arImage)
            {
                if($arImage["ID"])
                {
                    ...
                    $arOffer['MORE_PHOTO'][$i]["SMALL"] = CFile::ResizeImageGet($arImage["ID"], array(
                        "width"  => 340,
                        "height" => 340
                    ), BX_RESIZE_IMAGE_PROPORTIONAL, true, array());
                    $arOffer['MORE_PHOTO'][$i]["THUMB"] = CFile::ResizeImageGet($arImage["ID"], array(
                        "width"  => 56,
                        "height" => 56
                    ), BX_RESIZE_IMAGE_PROPORTIONAL, true, array());
                }
    ...
    if(!empty($arIDS) && CBXFeatures::IsFeatureEnabled('CatCompleteSet'))
    {
        ...
        $rsSets = CCatalogProductSet::getList(array(), array(
            '@OWNER_ID' => $arIDS,
            '=SET_ID'   => 0,
    ...
    if('TYPE_1' == $arParams['TYPE_SKU'] && $arResult['OFFERS'])
    {
        foreach($arResult['OFFERS'] as $keyOffer => $arOffer)
        {
            ...
            $arOneRow = array(
                ...
                /*(isset($arOffer['RATIO_PRICE']) ? $arOffer['RATIO_PRICE'] : $arOffer['MIN_PRICE']),*/
    ...
    if('TYPE_1' != $arParams['TYPE_SKU'] && $arResult['OFFERS'])
    {
        $arResult['MIN_PRICE'] = CMShop::getMinPriceFromOffersExt($arResult['OFFERS'], $boolConvert ? $arResult['CONVERT_CURRENCY']['CURRENCY_ID'] : $strBaseCurrency);
        $arTmpProps = array();
...
if(intVal($arParams["IBLOCK_STOCK_ID"]))
{
    ...
    $arResult["STOCK"] = CCache::CIBLockElement_GetList(array('CACHE' => array("TAG" => CCache::GetIBlockCacheTag($arParams["IBLOCK_STOCK_ID"]))), array(
...
if(!empty($arResult["PROPERTIES"]["SERVICES"]["VALUE"]))
{
    ...
    $arResult["SERVICES"] = CCache::CIBLockElement_GetList(array('CACHE' => array("TAG" => CCache::GetIBlockCacheTag($arResult["PROPERTIES"]["SERVICES"]["LINK_IBLOCK_ID"]))), array(
...
/*brand item*/
$arBrand = array();
$arResult["PROPERTIES"]["CML2_MANUFACTURER"]["LINK_IBLOCK_ID"] = 61;

if($arResult["PROPERTIES"]["CML2_MANUFACTURER"])
{
    if($arResult["PROPERTIES"]["CML2_MANUFACTURER"]["VALUE"] != '')
    {
        $arSelect = Array();
        $arFilter = Array(
            "IBLOCK_ID" => $arResult["PROPERTIES"]["CML2_MANUFACTURER"]["LINK_IBLOCK_ID"],
        ...
        if($arBrand)
        {
            ...
            {
                $arBrand["IMAGE"] = CFile::ResizeImageGet(($arBrand["PREVIEW_PICTURE"] ? $arBrand["PREVIEW_PICTURE"] : $arBrand["DETAIL_PICTURE"]), array(
                    "width"  => 120,
                    "height" => 40
...
/*stores product*/
$arStores = CCache::CCatalogStore_GetList(array(), array("ACTIVE" => "Y"), false, false, array());
$arResult["STORES_COUNT"] = count($arStores);

/**
 * 12/12/2016 Генерация псевдо предложений
 */
...
$arResult['QUANTITY_HTML'] = $arQuantityData['HTML'];

$arResult_tmp = array($arResult);
$arParams['DISPLAY_TYPE'] = 'detail';

\SB\PseudoSku::getSku($arResult_tmp, $arParams['IBLOCK_ID'], $arParams['DISPLAY_TYPE'], $arParams);
//$arResult = $arResult_tmp[0];
$arResult['SKU'] = $arResult_tmp[0]['SKU'];
