                ...
                else
                {
                    if($arResult["MORE_PHOTO"])
                    {
                        foreach($arResult["MORE_PHOTO"] as $i => $arImage)
                        {
                            ...
                            <? $APPLICATION->IncludeComponent("coffeediz:schema.org.ImageObject", "", Array(
                            "CAPTION"              => $alt,
                            "CONTENTURL"           => ($arImage["BIG"]["src"] != null) ? $arImage["BIG"]["src"] : $arImage["SRC"],
                            "DESCRIPTION"          => "",
                            ...
                            "SHOW"                 => "Y",
                            "TRUMBNAIL_CONTENTURL" => ($arImage["SMALL"]["src"] != null) ? $arImage["SMALL"]["src"] : $arImage["SRC"],
                            "URL"                  => "",
                            "WIDTH"                => $arImage['WIDTH']
                        )); ?>
                            <?
                                ...
                                <ul class="slides_block" id="<? echo $arItemIDs["ALL_ITEM_IDS"]['SLIDER_LIST_OF_ID'] . $arOneOffer['ID']; ?>">
                                    ...
                                    { ?>
                                        <li data-value="<? echo $arOneOffer['ID'] . '_' . $arOnePhoto['ID']; ?>" data-big="<?=$arOnePhoto['BIG']['src'];?>">
                                            <span class="cnt"><img border="0" src="<? echo $arOnePhoto['THUMB']['src']; ?>" alt="<?=$arOnePhoto["ALT"];?>" title="<?=$arOnePhoto["TITLE"];?>"/></span>
                                        </li>
                                        <?
                                    }
                                    unset($arOnePhoto); ?>
                    ...
                    <ul class="slides" id="<? echo $arItemIDs["ALL_ITEM_IDS"]['SLIDER_LIST_OFM_ID'] . $arOneOffer['ID']; ?>">
                        <? foreach($arOneOffer['MORE_PHOTO'] as &$arOnePhoto)
                        {
                            ...
                            <li data-value="<? echo $arOneOffer['ID'] . '_' . $arOnePhoto['ID']; ?>">
                                ...
                                ?>
                                <a href="<?=$arOnePhoto["BIG"]["src"]?>" rel="item_slider-<?=$arOneOffer['ID'];?>" class="fancy" title="<?=$title;?>">
                                    <img border="0" src="<?=$arOnePhoto["SMALL"]["src"]?>" alt="<?=$alt;?>" title="<?=$title;?>"/>
                                </a>
                            </li>
                            <?
                        }
    ...
    <div class="right_info">
        <div class="info_item">
            ...
            <div class="middle_info wrap_md">
                <div class="prices_block iblock">
                    <div class="cost prices clearfix">
                        ...
                        { ?>
                            ...
                            //_::d($arCountPricesCanAccess);

                          //  $arResult["PRICES"]['MIN_PRICE'] = $arResult["MIN_PRICE"];
                            foreach($arResult["PRICES"] as $key => $arPrice)
                            {
                ...
                <div class="buy_block iblock">
                    ...
                    <div class="buys_wrapp o_<?=$arOffer["ID"];?>" style="display: none;">
                        ...
                        <? if($arAddToBasketData["ACTION"] !== "NOTHING"): ?>
                                <? if($arAddToBasketData["ACTION"] == "ADD" && $arOffer["CAN_BUY"] && $arParams["SHOW_ONE_CLICK_BUY"] != "N"): ?>
                        <div class="wrapp_one_click">
											<span class="transparent big_btn type_block button one_click" data-offers="Y" data-item="<?=$arOffer["ID"]?>" data-iblockID="<?=$arOffer["IBLOCK_ID"]?>" data-quantity="<?=$arAddToBasketData["MIN_QUANTITY_BUY"];?>" data-props="<?=$arOfferProps?>" onclick="oneClickBuy('<?=$arOffer["ID"]?>', '<?=$arParams["IBLOCK_ID"]?>', this)">
												<span><?=GetMessage('ONE_CLICK_BUY')?></span>
											</span>
                        </div>
                        <? endif; ?>
            ...
            <div class="element_detail_text wrap_md">
                <div class="iblock sh">
                    <? $APPLICATION->IncludeFile(SITE_DIR . "/local/include/share.php", Array(), Array("MODE" => "html", "NAME" => GetMessage('CT_BCE_CATALOG_SOC_BUTTON'))); ?>
                </div>
...
<div class="tabs_section">
    <ul class="tabs1 main_tabs1 tabs-head">
        ...
        <? if($arResult['SKU'] && $arResult['PROPERTIES']['Z_COLOR']['VALUE']): ?>
                ...
                <div class="catalog_block">
                    <? foreach($arResult['SKU'] as &$arSku): ?>
							<?
                                ...
                                $APPLICATION->IncludeFile(SITE_TEMPLATE_PATH . "/includes/catalog/block_detail_element.php", Array("arItem" => $arSku, "arParams" => $arParams, 'arResult' => $arResult));
                            ?>
                    <? endforeach; ?>
                </div>
            </div>
...
<? $APPLICATION->IncludeComponent("coffeediz:schema.org.Product", "", Array("AGGREGATEOFFER" => "N", "DESCRIPTION" => $arResult['DETAIL_TEXT'], "ITEMAVAILABILITY" => "InStock", "ITEMCONDITION" => "NewCondition", "NAME" => $arResult['NAME'], "PARAM_RATING_SHOW" => "N", "PAYMENTMETHOD" => array(), "PRICE" => $arResult['PRICES']['BASE']['VALUE'], "PRICECURRENCY" => "RUB", "SHOW" => "Y")); ?>
