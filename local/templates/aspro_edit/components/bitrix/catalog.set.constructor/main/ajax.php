...
if ($_SERVER["REQUEST_METHOD"]=="POST" && strlen($_POST["action"])>0 && check_bitrix_sessid())
{
	...
	switch ($_POST["action"])
	{
		...
		case "ajax_recount_prices":
			...
			{
				$arPices = array("formatSum" => "", "formatOldSum" => "", "formatDiscDiffSum" => "", "formatDiscDiffPercent" => "");
				if ($_POST["sumPrice"])
				...
				if ($_POST["sumDiffDiscountPrice"]){
					...
					$arPices["formatDiscDiffPercent"]=round(((int)$_POST["sumDiffDiscountPrice"]/(int)$_POST["sumPrice"])*100, 2);
				}
