...
if(!empty($arResult['ITEMS']))
{
    ...
    foreach($arResult['ITEMS'] as $key => $arItem)
    {
        ...
        if($arItem['CATALOG'] && isset($arItem['OFFERS']) && !empty($arItem['OFFERS']))
        {
            ...
            {
                $arItem['MIN_PRICE'] = CMShop::getMinPriceFromOffersExt($arItem['OFFERS'], $boolConvert ? $arResult['CONVERT_CURRENCY']['CURRENCY_ID'] : $strBaseCurrency);
                $minItemPriceID = 0;
                $minItemPrice = 0;
                $minItemPriceFormat = "";
                foreach($arItem['OFFERS'] as $keyOffer => $arOffer)
        ...
        if($arResult['MODULES']['catalog'] && $arItem['CATALOG'] && ($arItem['CATALOG_TYPE'] == CCatalogProduct::TYPE_PRODUCT || $arItem['CATALOG_TYPE'] == CCatalogProduct::TYPE_SET))
        {
...
* ['PROPERTIES']['ZIP_IMAGE'] ['VALUE'] = 1
*/

    if($arItem['PROPERTIES']['ZIP_IMAGE'] ['VALUE'] != 1)
    {
        if($arItem['DETAIL_PICTURE']){
			$url_image_old_detail = 'https://' . $_SERVER['SERVER_NAME'] . $arItem['DETAIL_PICTURE']['SRC'];
			$url_image_old_prev = 'https://' . $_SERVER['SERVER_NAME'] . $arItem['PREVIEW_PICTURE']['SRC'];
			//$newLink_image = getLink_compres_image($url_image_old);
			CModule::IncludeModule('iblock');
			$el = new CIBlockElement;
			//$c_Make_file = CFile::MakeFileArray($newLink_image);
			$c_Make_file_detail = CFile::MakeFileArray('https://img.gs/vfgcsltwgv/full/' . $url_image_old_detail);
			$c_Make_file_prev = CFile::MakeFileArray('https://img.gs/vfgcsltwgv/full/' . $url_image_old_prev);
			$arLoadProductArray = Array();
			$arLoadProductArray['DETAIL_PICTURE'] = $c_Make_file_detail;
			$arLoadProductArray['PREVIEW_PICTURE'] = $c_Make_file_prev;
			$arLoadProductArray['PROPERTY_VALUES'] = array('ZIP_IMAGE' => 1);
			$res = $el->Update($arItem['ID'], $arLoadProductArray);

			//$arResult['ITEMS'][$key]['DETAIL_PICTURE']['SRC'] = substr($c_Make_file_detail['tmp_name'],strpos($c_Make_file_detail['tmp_name'],'shop.mehendi24.ru'), strlen($c_Make_file_detail['tmp_name']));
			$arResult['ITEMS'][$key]['DETAIL_PICTURE'] = $c_Make_file_detail;
			//$arResult['ITEMS'][$key]['PREVIEW_PICTURE']['SRC'] = $arResult['ITEMS'][$key]['DETAIL_PICTURE']['SRC'];
		}
    }
