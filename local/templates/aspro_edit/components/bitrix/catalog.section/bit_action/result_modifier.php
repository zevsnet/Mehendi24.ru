...
$rsSections = CIBlockSection::GetList(array('SORT'=>'ASC', 'NAME' => 'ASC'), $arFilter, false, Array("ID","NAME","SECTION_PAGE_URL"), false);
$key=0;
...
while ($arSction = $rsSections->GetNext())
{
	...
	$filter=Array("IBLOCK_ID" => $arParams[IBLOCK_ID], "SECTION_ID"=>$arSction[ID], "PROPERTY_CML2_MANUFACTURER"=>$_POST[FILTER][BRANDS]);
	$dbElements = CIBlockElement::GetList(array("CATALOG_PRICE_2"=>"ASC"), $filter, false, array('nTopCount'=>1), Array('ID', 'CATALOG_PRICE_2'));
	...
	while($arrElement = $dbElements->GetNext()) 
	{
		...
		if ($arrElement['PREVIEW_PICTURE']) break;
...
if (CModule::IncludeModule('catalog'))
{
	...
	if ($arDiscountID)
	{
		...
		$dbElements = CIBlockElement::GetList(array("SORT"=>"ASC"), $filter, false, false, Array('ID', 'NAME', 'PROPERTY_SALE_ID'));
		while($arrElement = $dbElements->GetNext()) 
		{
