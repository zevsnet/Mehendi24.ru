    ...
    $(function()
    {
		...
		$(".l_minus").click(function()
        {
            if($(this).parent().find(".l_count").val() > 1)
            {
				...
				if($(this).parent().find(".l_count").val()>99){$(this).parent().find(".l_count").css({'width':'35px'})}
				else {$(this).parent().find(".l_count").css({'width':'20px'})}
				if(Number($('#srcart span').html())>99){$("#srcart span").css({'width':'35px'});}else{$("#srcart span").css({'width':'20px'});}
            }
        });
		...
		$('.l_count').change(function(){
			...
			if($(this).parent().find(".l_count").val()>99){$(this).parent().find(".l_count_top").css({'width':'35px'})}
				else {$(this).parent().find(".l_count").css({'width':'20px'})}
			if(Number($('#srcart span').html())>99){$("#srcart span").css({'width':'35px'});}else{$("#srcart span").css({'width':'20px'});}
		});
        ...
        {
            if($(".catsearchfield").hasClass("searchact"))
            {
                $(".catsearchfield").fadeOut(50).removeClass("searchact");
            }
            else
            {
                $(".catsearchfield").fadeIn(50).addClass("searchact");
            }
...
if ($arParentSection = $rsParentSection->GetNext())
{
    ...
    while ($arSect = $rsSect->GetNext())
    {
        ...
        if($add_section)$section_Result[]=$arSect;
    }

    $iCnt = CIBlockElement::GetList(array(),array("IBLOCK_ID"=>$arParentSection['IBLOCK_ID'],"ACTIVE"=>"Y","SECTION_ID"=>$GLOBALS['SECTION_POS']['ID']),array());
...
<div class="listproduct_action">

    <?include 'include/view.php'?>

    <div style="padding-left: 5px;display: table;width:100%;">
        <?
        ...
        if ($arResult['ITEMS']) {
            foreach ($arResult['ITEMS'] as $key => $arItem):
                ...
                if ($catshowtype == "CARD") {
                $style_tmp='';
                if (($key + 1) / $arParams['LINE_ELEMENT_COUNT'] - floor(($key + 1) / $arParams['LINE_ELEMENT_COUNT']) != 0) {
                    $style_tmp = 'l_card_right';
                }
                $class_tmp ='l_card';
...
margin-top: 15px;margin-bottom: 6px;">
                                    ...
                                    <input type="hidden" value="<?=$arItem['MIN_PRICE']['VALUE']
                                    - $arItem['MIN_PRICE']['DISCOUNT_VALUE']?>" name="pprice_disc"/>
...
font-weight: bold;
cursor: pointer;
text-decoration: underline;" class="saddtocart" rel="<?=$arItem[ID]?>">В КОРЗИНУ
                                    </div>
                                    <?
                                } else {
                                    ?>
                            ...
                            <a href="<? echo $arItem['DETAIL_PAGE_URL']; ?>">
                                <div class="l_list_img" style="background-image: url(<?=$picture?>);">
                                    <? if ($arItem['MIN_PRICE']['VALUE']
                                        ...
                                        ?>
                                        <i class="l_discount"><input type="hidden" value="<?=$arItem['discount']['ID']?>"/></i>
                                        <?
                                    } ?>
                                </div>
                            </a>
...
color: #878787;"><? echo $arItem[PROPERTIES][CML2_MANUFACTURER][VALUE]; ?></div>
                                    <div style="font-weight: bold;"><?=$arItem[NAME]?></div>
                                    <p><? if (strlen($arItem[DETAIL_TEXT])
                                            > 0
                                        ) {
                                            echo substr(
                                                $arItem["DETAIL_TEXT"], 0, 200
                                            );
