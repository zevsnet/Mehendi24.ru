...
if(!empty($arResult['ITEMS']))
{
    ...
    if($arResult['MODULES']['catalog'])
    {
        ...
        {
            $arSKUPropList = CIBlockPriceTools::getTreeProperties($arSKU, $arParams['OFFER_TREE_PROPS'], array(
                'PICT' => $arEmptyPreview,
                'NAME' => '-'
            ));
    ...
    foreach($arResult['ITEMS'] as $key => $arItem)
    {
        ...
        if('' != $arParams['LABEL_PROP'] && isset($arItem['PROPERTIES'][$arParams['LABEL_PROP']]))
        {
            ...
            if(!empty($arProp['VALUE']))
            {
                ...
                {
                    $arItem['LABEL_VALUE'] = (is_array($arProp['VALUE']) ? implode(' / ', $arProp['VALUE']) : $arProp['VALUE']);
                }
                $arItem['LABEL'] = true;
...
/****Добавляем в список товаров раздел 4го уровня****/
if($GLOBALS[subsection] == "N")
{
    $arFilter = array('IBLOCK_ID' => $arParams["IBLOCK_ID"], "SECTION_ID" => $arResult[ID]);
    $rsSections = CIBlockSection::GetList(array('SORT' => 'ASC', 'NAME' => 'ASC'), $arFilter, false, Array(
        "ID",
        "NAME",
        "SECTION_PAGE_URL"
    ), false);
    ...
    while($arSction = $rsSections->GetNext())
    {
        ...
        $filter = Array(
            ...
            "SECTION_ID"                 => $arSction[ID],
            "PROPERTY_CML2_MANUFACTURER" => $_POST[FILTER][BRANDS]
        );
        $dbElements = CIBlockElement::GetList(array("CATALOG_PRICE_2" => "ASC"), $filter, false, array('nTopCount' => 1), Array(
            'ID',
            'CATALOG_PRICE_2'
        ));
        while($arrElement = $dbElements->GetNext())
        {
            if($arrElement[CATALOG_PRICE_2])
            {
                //$arResult4[ITEMS][$key]['MIN_PRICE']['VALUE'] = "от&nbsp;".$arrElement[CATALOG_PRICE_2];
                $arResult4[ITEMS][$key]['MIN_PRICE']['PRINT_DISCOUNT_VALUE'] = "от&nbsp;" . FormatCurrency($arrElement[CATALOG_PRICE_2], "RUB");
                $show = 1;
            }
        }

        $dbElements = CIBlockElement::GetList(array('NAME' => 'ASC'), $filter, false, false, Array('PREVIEW_PICTURE'));
        while($arrElement = $dbElements->GetNext())
        {

            $arResult4[ITEMS][$key][PREVIEW_PICTURE][SRC] = CFile::GetPath($arrElement['PREVIEW_PICTURE']);
            if($arrElement['PREVIEW_PICTURE'])
                break;
        }
        if($show == 1)
        {
            $arResult4[ITEMS][$key][NAME] = $arSction[NAME];
            $arResult4[ITEMS][$key][DETAIL_PAGE_URL] = $arSction[SECTION_PAGE_URL];
            $arResult4[ITEMS][$key][NOBASKET] = "Y";
            $key++;
        }
    }
    if($arResult4[ITEMS])
    {
        $arResult[ITEMS] = array_merge($arResult4[ITEMS], $arResult[ITEMS]);
    }
    //global $USER;
} ?>
...
if(CModule::IncludeModule('catalog'))
{
    foreach($arResult['ITEMS'] as $key => $arItem)
    {
        ...
        if($arItem['MIN_PRICE']['VALUE'] != $arItem['MIN_PRICE']['DISCOUNT_VALUE'])
        {
            ...
            $arDiscounts = CCatalogDiscount::GetDiscountByProduct($arItem['ID'], array(
                1,
                2,
                3,
                4,
            ...
            if($arDiscounts)
            {
                foreach($arDiscounts as $arDiscount)
                    $arDiscountID[$arDiscount['ID']][] = $key;
            }
        }
    }
    ...
    {
        echo '<!--';
        print_r($arDiscountID);
        echo '-->';

        $filter = Array("IBLOCK_ID" => 21, "PROPERTY_SALE_ID" => array_keys($arDiscountID), 'ACTIVE' => 'Y');
        $dbElements = CIBlockElement::GetList(array("SORT" => "ASC"), $filter, false, false, Array(
            'ID',
            'NAME',
            'PROPERTY_SALE_ID'
        ));
        while($arrElement = $dbElements->GetNext())
        {
            ...
            $id = $arrElement['PROPERTY_SALE_ID_VALUE'];

            foreach($arDiscountID[$id] as $key)
            {
                $arResult['ITEMS'][$key]['discount'] = $arrElement;
            }
        }
    }
}
...
if(is_array($arResult['ITEMS']) && count($arResult['ITEMS']) > 0)
{
    foreach($arResult['ITEMS'] as $key => $item)
    {
        ...
        $arFileBig = CFile::ResizeImageGet($item['DETAIL_PICTURE'], array(
            "width"  => 150,
            "height" => 150
        ), BX_RESIZE_IMAGE_PROPORTIONAL, true, $arWaterMark);

        $item['DETAIL_PICTURE']['WIDTH']=$arFileBig['width'];
        $item['DETAIL_PICTURE']['HEIGHT']=$arFileBig['height'];
        $item['DETAIL_PICTURE']['SRC']=$arFileBig['src'];
        $arResult['ITEMS'][$key]['PREVIEW_PICTURE']=$arResult['ITEMS'][$key]['DETAIL_PICTURE']=$item['DETAIL_PICTURE'];
/*
        $arFilePRE = CFile::ResizeImageGet($item['PREVIEW_PICTURE'], array(
            "width"  => 150,
            "height" => 150
        ), BX_RESIZE_IMAGE_PROPORTIONAL, true, $arWaterMark);

        $item['PREVIEW_PICTURE']['WIDTH']=$arFilePRE['width'];
        $item['PREVIEW_PICTURE']['HEIGHT']=$arFilePRE['height'];
        $item['PREVIEW_PICTURE']['SRC']=$arFilePRE['src'];
        $arResult['ITEMS'][$key]['PREVIEW_PICTURE']=$item['PREVIEW_PICTURE'];*/
    }
}
?>
