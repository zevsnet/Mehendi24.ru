...
if (!empty($arResult['ITEMS'])){
	...
	{
		$arItemIDs=CMShop::GetItemsIDs($arItems, "Y");
		$totalCount = CMShop::GetTotalCount($arItems);
		$arQuantityData = CMShop::GetQuantityArray($totalCount, $arItemIDs["ALL_ITEM_IDS"], "Y");

		$arItems['QUANTITY_HTML'] = $arQuantityData['HTML'];
	}
	...
	{
		if(!$arResultItems[$arItem['PROPERTIES'][$arParams['GROUP_BY']]['VALUE']])
			$arResultItems[$arItem['PROPERTIES'][$arParams['GROUP_BY']]['VALUE']] = $arItem;
		$arResultItems[$arItem['PROPERTIES'][$arParams['GROUP_BY']]['VALUE']]['SKU'][] = $arItem;
	}

	$arResult['ITEMS'] = $arResultItems;

	//\SB\General::getSku($arResult['ITEMS'], $arResult['IBLOCK_ID'], $arParams['DISPLAY_TYPE'], $arParams);
	foreach($arResult['ITEMS'] as &$arItem)
	{
		$arItem['SKU_TEMPLATE'] = \SB\General::getSkuTemplate($arItem['SKU'], $arResult['IBLOCK_ID'], $arItem['ID'], $arParams['DISPLAY_TYPE'], $arItem['QUANTITY_HTML']);
	}
