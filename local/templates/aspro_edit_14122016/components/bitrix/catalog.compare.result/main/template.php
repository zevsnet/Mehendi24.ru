...
<div class="bx_sort_container">
	...
	<span class="wrap_remove_button">
		...
		$arStr=implode("&ID[]=", $compare_items)?>
		<span class="button grey_br transparent remove_all_compare icon_close" onclick="CatalogCompareObj.MakeAjaxAction('/catalog/compare.php?action=DELETE_FROM_COMPARE_RESULT&ID[]=<?=$arStr?>');"><?=GetMessage("CLEAR_ALL_COMPARE")?></span>
	</span>
</div>
						...
						<?foreach($arResult["ITEMS"] as &$arElement){?>
							<td>
								<div class="item_block">
									<span onclick="CatalogCompareObj.MakeAjaxAction('<?=CUtil::JSEscape($arElement['~DELETE_URL'])?>', '<?=$arElement['ID']?>', '<?=$arElement['IBLOCK_ID']?>');" class="remove" title="<?=GetMessage("CATALOG_REMOVE_PRODUCT")?>"><i></i></span>
		...
		foreach ($arResult["SHOW_FIELDS"] as $code => $arProp){
			if(!in_array($code, $arUnvisible)){
				...
				if ($arResult['DIFFERENT']){
					...
					unset($arElement);
					$showFRow = (count(array_unique($arCompare)) > 1);
				}
			}
		}
	}
		...
		foreach ($arResult["SHOW_OFFER_FIELDS"] as $code => $arProp){
			if(!in_array($code, $arUnvisible)){
				...
				if ($arResult['DIFFERENT']){
					...
					unset($arElement);
					$showOFRow = (count(array_unique($arCompare)) > 1);
				}
			}
		}
	}
	if (!empty($arResult["SHOW_PROPERTIES"])){
		foreach ($arResult["SHOW_PROPERTIES"] as $code => $arProperty){
			...
			if ($arResult['DIFFERENT']){
				...
				unset($arElement);
				$showPRow = (count(array_unique($arCompare)) > 1);
			}
		}
	}
	if (!empty($arResult["SHOW_OFFER_PROPERTIES"])){
		foreach ($arResult["SHOW_OFFER_PROPERTIES"] as $code => $arProperty){
			...
			if ($arResult['DIFFERENT']){
				...
				unset($arElement);
				$showOPRow = (count(array_unique($arCompare)) > 1);
			}
		}
	}*/
	//if($showFRow|| $showOFRow || $showPRow || $showOPRow){?>
