...
foreach($arResult['ITEMS'] as $key => $arItem)
{
    ...
    if($arItem['PROPERTIES']['ZIP_IMAGE'] ['VALUE'] != 1)
    {
        if($arItem['DETAIL_PICTURE'])
        {
            ...
            $c_Make_file_detail = CFile::MakeFileArray('https://img.gs/' . $user_api . '/full/' . $url_image_old_detail);
            $c_Make_file_prev = CFile::MakeFileArray('https://img.gs/' . $user_api . '/full/' . $url_image_old_prev);
