		...
		if (!empty($arSource))
		{
			?>
				<div class="wrap_props">
					...
					{
						?>
						<div data-property-id-row="<?=intval(intval($arProperties["ID"]))?>" class="prop_block">
						...
						elseif ($arProperties["TYPE"] == "SELECT")
						{
							...
							<div class="bx_block r3x1">
								<select class="<?=($arProperties["REQUIED_FORMATED"]== "Y" ? 'required' : '')?>" code="<?=$arProperties["CODE"]?>" name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>" size="<?=$arProperties["SIZE1"]?>">
									<?
						...
						elseif ($arProperties["TYPE"] == "MULTISELECT")
						{
							...
							<div class="bx_block r3x1">
								<select multiple class="<?=($arProperties["REQUIED_FORMATED"]== "Y" ? 'required' : '')?>" code="<?=$arProperties["CODE"]?>" name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>" size="<?=$arProperties["SIZE1"]?>">
									<?
									foreach($arProperties["VARIANTS"] as $arVariants):
									?>
										<option value="<?=$arVariants["VALUE"]?>"<?if ($arVariants["SELECTED"] == "Y") echo " selected";?>><?=$arVariants["NAME"]?></option>
						...
						elseif ($arProperties["TYPE"] == "TEXTAREA")
						{
							...
							<div class="bx_block r3x1">
								<textarea class="<?=($arProperties["REQUIED_FORMATED"]== "Y" ? 'required' : '')?>" code="<?=$arProperties["CODE"]?>" rows="<?=$rows?>" cols="<?=$arProperties["SIZE1"]?>" name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>"><?=$arProperties["VALUE"]?></textarea>
						...
						elseif ($arProperties["TYPE"] == "LOCATION")
						{
							...
							<div class="bx_block r3x1">
								...
								<?if($locationTemplateP == 'steps'):?>
									<input type="hidden" class="<?=($arProperties["REQUIED_FORMATED"]== "Y" ? 'required' : '')?>" code="<?=$arProperties["CODE"]?>" id="LOCATION_ALT_PROP_DISPLAY_MANUAL[<?=intval($arProperties["ID"])?>]" name="LOCATION_ALT_PROP_DISPLAY_MANUAL[<?=intval($arProperties["ID"])?>]" value="<?=($_REQUEST['LOCATION_ALT_PROP_DISPLAY_MANUAL'][intval($arProperties["ID"])] ? '1' : '0')?>" />
								<?endif?>
